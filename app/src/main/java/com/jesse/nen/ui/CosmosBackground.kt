...
private fun CosmosFluid(image: ImageBitmap, modifier: Modifier) {
    ...
    Canvas(modifier = modifier.fillMaxSize()) {
        ...
        shader.setFloatUniform("u_halfSize", size.width / 2f, size.height / 2f)
...
half4 main(float2 fragCoord) {
    ...
    // Quatre copies des volutes, une par coin de l'écran, qui dérivent et tournent lentement
    // chacune à son rythme. Elles se recouvrent sans jamais se remettre en phase, ce qui les
    // empêche de se lire comme la même image répétée.
    //
    // Positionnées en fraction de u_halfSize (pas en pixels absolus) : sur tout format d'écran,
    // chaque copie reste ancrée près d'un coin dès l'ouverture, sans attendre que la dérive
    // l'amène dans le cadre.
    //
    ...
    float hw = u_halfSize.x;
    float hh = u_halfSize.y;
    half3 extra = half3(0.0);
    extra += nebulaCopy(d, float2(-hw * 0.62, -hh * 1.15 + sin(drift) * hh * 0.07), 0.7 + drift, 1.20) * 0.62;
    extra += nebulaCopy(d, float2(-hw * 2.05 + cos(drift * 1.3) * hw * 0.11, hh * 0.46), 4.1 + drift * 0.5, 1.35) * 0.50;
    extra += nebulaCopy(d, float2(hw * 2.05 - sin(drift * 1.1) * hw * 0.11, -hh * 0.46), 1.6 - drift * 0.4, 1.15) * 0.55;
    // Les deux copies suivantes visent pile les coins bas : zoom serré, donc réveil de la
    // matière dès une faible distance au centre de la copie — sans lui, le rayon mort du
    // masque (proportionnel au zoom) dépasse la distance écran-coin et rien n'apparaît là.
    // Rotation quasi nulle : le décalage pointant en diagonale au-delà du coin, le vecteur
    // du coin vers la copie pointe déjà vers le centre de l'image (là où la matière existe) —
